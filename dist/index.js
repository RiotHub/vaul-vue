(function(){"use strict";try{if(typeof document<"u"){var r=document.createElement("style");r.appendChild(document.createTextNode('[vaul-drawer]{touch-action:none;will-change:transform;transition:transform .5s cubic-bezier(.32,.72,0,1)}[vaul-drawer][vaul-drawer-direction=bottom]{transform:translate3d(0,100%,0)}[vaul-drawer][vaul-drawer-direction=top]{transform:translate3d(0,-100%,0)}[vaul-drawer][vaul-drawer-direction=left]{transform:translate3d(-100%,0,0)}[vaul-drawer][vaul-drawer-direction=right]{transform:translate3d(100%,0,0)}.vaul-dragging .vaul-scrollable [vault-drawer-direction=top],.vaul-dragging .vaul-scrollable [vault-drawer-direction=bottom]{overflow-y:hidden!important}.vaul-dragging .vaul-scrollable [vault-drawer-direction=left],.vaul-dragging .vaul-scrollable [vault-drawer-direction=right]{overflow-x:hidden!important}[vaul-drawer][vaul-drawer-visible=true][vaul-drawer-direction=top],[vaul-drawer][vaul-drawer-visible=true][vaul-drawer-direction=bottom]{transform:translate3d(0,var(--snap-point-height, 0),0)}[vaul-drawer][vaul-drawer-visible=true][vaul-drawer-direction=left],[vaul-drawer][vaul-drawer-visible=true][vaul-drawer-direction=right]{transform:translate3d(var(--snap-point-height, 0),0,0)}[vaul-overlay]{opacity:0;transition:opacity .5s cubic-bezier(.32,.72,0,1)}[vaul-overlay][vaul-drawer-visible=true]{opacity:1}[vaul-drawer]:after{content:"";position:absolute;background:inherit;background-color:inherit}[vaul-drawer][vaul-drawer-direction=top]:after{top:initial;bottom:100%;left:0;right:0;height:200%}[vaul-drawer][vaul-drawer-direction=bottom]:after{top:100%;bottom:initial;left:0;right:0;height:200%}[vaul-drawer][vaul-drawer-direction=left]:after{left:initial;right:100%;top:0;bottom:0;width:200%}[vaul-drawer][vaul-drawer-direction=right]:after{left:100%;right:initial;top:0;bottom:0;width:200%}[vaul-overlay][vaul-snap-points=true]:not([vaul-snap-points-overlay=true]):not([data-state=closed]){opacity:0}[vaul-overlay][vaul-snap-points-overlay=true]:not([vaul-drawer-visible=false]){opacity:1}@keyframes fake-animation{}@media (hover: hover) and (pointer: fine){[vaul-drawer]{-webkit-user-select:none;user-select:none}}')),document.head.appendChild(r)}}catch(a){console.error("vite-plugin-css-injected-by-js",a)}})();
import { getCurrentInstance as We, ref as P, watch as Y, nextTick as Oe, computed as V, onMounted as ze, onUnmounted as Te, watchEffect as ke, defineComponent as le, useSlots as Ie, toRefs as qe, openBlock as oe, createBlock as ie, unref as C, withCtx as se, renderSlot as ce, mergeProps as Ye, normalizeStyle as Xe } from "vue";
import { createContext as Ge, DialogRoot as Je, useForwardPropsEmits as Ke, DialogOverlay as Qe, DialogContent as Ze } from "radix-vue";
import { DialogClose as bt, DialogDescription as $t, DialogPortal as yt, DialogTitle as Dt, DialogTrigger as St } from "radix-vue";
const Fe = typeof window < "u" && typeof document < "u";
typeof WorkerGlobalScope < "u" && globalThis instanceof WorkerGlobalScope;
const et = (t) => typeof t < "u";
function tt(t) {
  return JSON.parse(JSON.stringify(t));
}
function ye(t, n, d, u = {}) {
  var e, b, s;
  const {
    clone: O = !1,
    passive: x = !1,
    eventName: v,
    deep: l = !1,
    defaultValue: w,
    shouldEmit: R
  } = u, A = We(), y = d || (A == null ? void 0 : A.emit) || ((e = A == null ? void 0 : A.$emit) == null ? void 0 : e.bind(A)) || ((s = (b = A == null ? void 0 : A.proxy) == null ? void 0 : b.$emit) == null ? void 0 : s.bind(A == null ? void 0 : A.proxy));
  let i = v;
  n || (n = "modelValue"), i = i || `update:${n.toString()}`;
  const r = (p) => O ? typeof O == "function" ? O(p) : tt(p) : p, o = () => et(t[n]) ? r(t[n]) : w, f = (p) => {
    R ? R(p) && y(i, p) : y(i, p);
  };
  if (x) {
    const p = o(), T = P(p);
    let D = !1;
    return Y(
      () => t[n],
      (S) => {
        D || (D = !0, T.value = r(S), Oe(() => D = !1));
      }
    ), Y(
      T,
      (S) => {
        !D && (S !== t[n] || l) && f(S);
      },
      { deep: l }
    ), T;
  } else
    return V({
      get() {
        return o();
      },
      set(p) {
        f(p);
      }
    });
}
const [de, nt] = Ge("DrawerRoot"), Pe = /* @__PURE__ */ new WeakMap();
function $(t, n, d = !1) {
  if (!t || !(t instanceof HTMLElement) || !n)
    return;
  const u = {};
  Object.entries(n).forEach(([e, b]) => {
    if (e.startsWith("--")) {
      t.style.setProperty(e, b);
      return;
    }
    u[e] = t.style[e], t.style[e] = b;
  }), !d && Pe.set(t, u);
}
function ne(t, n) {
  if (!t || !(t instanceof HTMLElement))
    return;
  const d = Pe.get(t);
  d && (n ? t.style[n] = d[n] : Object.entries(d).forEach(([u, e]) => {
    t.style[u] = e;
  }));
}
function ae(t, n) {
  const d = window.getComputedStyle(t), u = d.transform || d.webkitTransform || d.mozTransform;
  let e = u.match(/^matrix3d\((.+)\)$/);
  return e ? Number.parseFloat(e[1].split(", ")[N(n) ? 13 : 12]) : (e = u.match(/^matrix\((.+)\)$/), e ? Number.parseFloat(e[1].split(", ")[N(n) ? 5 : 4]) : null);
}
function at(t) {
  return 8 * (Math.log(t + 1) - 2);
}
function N(t) {
  switch (t) {
    case "top":
    case "bottom":
      return !0;
    case "left":
    case "right":
      return !1;
    default:
      return t;
  }
}
const g = {
  DURATION: 0.5,
  EASE: [0.32, 0.72, 0, 1]
}, Re = 0.4;
function lt({
  activeSnapPoint: t,
  snapPoints: n,
  drawerRef: d,
  overlayRef: u,
  fadeFromIndex: e,
  onSnapPointChange: b,
  direction: s
}) {
  const O = V(
    () => (n.value && t.value === n.value[n.value.length - 1]) ?? null
  ), x = V(
    () => n.value && n.value.length > 0 && ((e == null ? void 0 : e.value) || (e == null ? void 0 : e.value) === 0) && !Number.isNaN(e == null ? void 0 : e.value) && n.value[(e == null ? void 0 : e.value) ?? -1] === t.value || !n.value
  ), v = V(
    () => {
      var r;
      return ((r = n.value) == null ? void 0 : r.findIndex((o) => o === t.value)) ?? null;
    }
  ), l = V(
    () => {
      var r;
      return ((r = n.value) == null ? void 0 : r.map((o) => {
        const f = typeof window < "u", p = typeof o == "string";
        let T = 0;
        if (p && (T = Number.parseInt(o, 10)), N(s.value)) {
          const S = p ? T : f ? o * window.innerHeight : 0;
          return f ? s.value === "bottom" ? window.innerHeight - S : -window.innerHeight + S : S;
        }
        const D = p ? T : f ? o * window.innerWidth : 0;
        return f ? s.value === "right" ? window.innerWidth - D : -window.innerWidth + D : D;
      })) ?? [];
    }
  ), w = V(
    () => {
      var r;
      return v.value !== null ? (r = l.value) == null ? void 0 : r[v.value] : null;
    }
  ), R = (r) => {
    var f, p, T, D;
    const o = ((f = l.value) == null ? void 0 : f.findIndex((S) => S === r)) ?? null;
    Oe(() => {
      var S;
      b(o, l.value), $((S = d.value) == null ? void 0 : S.$el, {
        transition: `transform ${g.DURATION}s cubic-bezier(${g.EASE.join(",")})`,
        transform: N(s.value) ? `translate3d(0, ${r}px, 0)` : `translate3d(${r}px, 0, 0)`
      });
    }), l.value && o !== l.value.length - 1 && o !== (e == null ? void 0 : e.value) ? $((p = u.value) == null ? void 0 : p.$el, {
      transition: `opacity ${g.DURATION}s cubic-bezier(${g.EASE.join(",")})`,
      opacity: "0"
    }) : $((T = u.value) == null ? void 0 : T.$el, {
      transition: `opacity ${g.DURATION}s cubic-bezier(${g.EASE.join(",")})`,
      opacity: "1"
    }), t.value = o !== null ? ((D = n.value) == null ? void 0 : D[o]) ?? null : null;
  };
  Y(
    [t, l, n],
    () => {
      var r;
      if (t.value) {
        const o = ((r = n.value) == null ? void 0 : r.findIndex((f) => f === t.value)) ?? -1;
        l.value && o !== -1 && typeof l.value[o] == "number" && R(l.value[o]);
      }
    },
    {
      immediate: !0
      // if you want to run the effect immediately as well
    }
  );
  function A({
    draggedDistance: r,
    closeDrawer: o,
    velocity: f,
    dismissible: p
  }) {
    var W, X, z;
    if (e === void 0)
      return;
    const T = s.value === "bottom" || s.value === "right" ? (w.value ?? 0) - r : (w.value ?? 0) + r, D = v.value === (e.value ?? 0) - 1, S = v.value === 0, L = r > 0;
    if (D && $((W = u.value) == null ? void 0 : W.$el, {
      transition: `opacity ${g.DURATION}s cubic-bezier(${g.EASE.join(",")})`
    }), f > 2 && !L) {
      p ? o() : R(l.value[0]);
      return;
    }
    if (f > 2 && L && l && n.value) {
      R(l.value[n.value.length - 1]);
      return;
    }
    const Z = (X = l.value) == null ? void 0 : X.reduce((U, c) => typeof U != "number" || typeof c != "number" ? U : Math.abs(c - T) < Math.abs(U - T) ? c : U), j = N(s.value) ? window.innerHeight : window.innerWidth;
    if (f > Re && Math.abs(r) < j * 0.4) {
      const U = L ? 1 : -1;
      if (U > 0 && O) {
        R(l.value[(((z = n.value) == null ? void 0 : z.length) ?? 0) - 1]);
        return;
      }
      if (S && U < 0 && p && o(), v.value === null)
        return;
      R(l.value[v.value + U]);
      return;
    }
    R(Z);
  }
  function y({ draggedDistance: r }) {
    var f;
    if (w.value === null)
      return;
    const o = s.value === "bottom" || s.value === "right" ? (w.value ?? 0) - r : (w.value ?? 0) + r;
    (s.value === "bottom" || s.value === "right") && o < l.value[l.value.length - 1] || (s.value === "top" || s.value === "left") && o > l.value[l.value.length - 1] || $((f = d.value) == null ? void 0 : f.$el, {
      transform: N(s.value) ? `translate3d(0, ${o}px, 0)` : `translate3d(${o}px, 0, 0)`
    });
  }
  function i(r, o) {
    if (!n || typeof v.value != "number" || !l.value || e === void 0)
      return null;
    const f = v.value === (e.value ?? 0) - 1;
    if (v.value >= (e.value ?? 0) && o)
      return 0;
    if (f && !o)
      return 1;
    if (!x.value && !f)
      return null;
    const T = f ? v.value + 1 : v.value - 1, D = f ? l.value[T] - l.value[T - 1] : l.value[T + 1] - l.value[T], S = r / Math.abs(D);
    return f ? 1 - S : S;
  }
  return {
    isLastSnapPoint: O,
    shouldFade: x,
    getPercentageDragged: i,
    activeSnapPointIndex: v,
    onRelease: A,
    onDrag: y,
    snapPointsOffset: l
  };
}
let Q = null;
function ot(t) {
  const { isOpen: n, modal: d, nested: u, hasBeenOpened: e } = t, b = P(typeof window < "u" ? window.location.href : ""), s = P(0);
  function O() {
    if (Q === null && n.value) {
      Q = {
        position: document.body.style.position,
        top: document.body.style.top,
        left: document.body.style.left,
        height: document.body.style.height
      };
      const { scrollX: v, innerHeight: l } = window;
      document.body.style.position = "fixed", document.body.style.top = `-${s.value}px`, document.body.style.left = `-${v}px`, document.body.style.right = "0px", document.body.style.height = "auto", setTimeout(() => {
        requestAnimationFrame(() => {
          const w = l - window.innerHeight;
          w && s.value >= l && (document.body.style.top = `-${s.value + w}px`);
        });
      }, 300);
    }
  }
  function x() {
    if (Q !== null) {
      const v = -Number.parseInt(document.body.style.top, 10), l = -Number.parseInt(document.body.style.left, 10);
      Object.assign(document.body.style, Q), requestAnimationFrame(() => {
        if (b.value !== window.location.href) {
          b.value = window.location.href;
          return;
        }
        window.scrollTo(l, v);
      }), Q = null;
    }
  }
  return ze(() => {
    function v() {
      s.value = window.scrollY;
    }
    v(), window.addEventListener("scroll", v), Te(() => {
      window.removeEventListener("scroll", v);
    });
  }), Y([n, e, b], () => {
    u.value || !e.value || (n.value ? (!window.matchMedia("(display-mode: standalone)").matches && O(), d.value || setTimeout(() => {
      x();
    }, 500)) : x());
  }), { restorePositionSetting: x };
}
const it = 0.25, rt = 100, De = 8, q = 16, ut = 26, Se = "vaul-dragging";
function st(t, n) {
  return t && t.value ? t : n;
}
function ct(t) {
  const {
    emitDrag: n,
    emitRelease: d,
    emitClose: u,
    emitOpenChange: e,
    open: b,
    dismissible: s,
    nested: O,
    fixed: x,
    modal: v,
    shouldScaleBackground: l,
    scrollLockTimeout: w,
    closeThreshold: R,
    activeSnapPoint: A,
    fadeFromIndex: y,
    direction: i
  } = t, r = P(b.value ?? !1), o = P(!1), f = P(!1), p = P(!1), T = P(!1), D = P(null), S = P(null), L = P(null), Z = P(null), j = P(null), W = P(!0), X = P(null), z = P(0), U = P(!1);
  P(0);
  const c = P(null);
  P(0);
  const ve = V(() => {
    var a;
    return ((a = c.value) == null ? void 0 : a.$el.getBoundingClientRect().height) || 0;
  }), H = st(
    t.snapPoints,
    P(void 0)
  ), {
    activeSnapPointIndex: fe,
    onRelease: Ee,
    snapPointsOffset: Ne,
    onDrag: Ae,
    shouldFade: pe,
    getPercentageDragged: _e
  } = lt({
    snapPoints: H,
    activeSnapPoint: A,
    drawerRef: c,
    fadeFromIndex: y,
    overlayRef: D,
    onSnapPointChange: Ce,
    direction: i
  });
  function Ce(a, m) {
    H.value && a === m.length - 1 && (S.value = /* @__PURE__ */ new Date());
  }
  const { restorePositionSetting: xe } = ot({
    isOpen: r,
    modal: v,
    nested: O,
    hasBeenOpened: o
  });
  function G() {
    return (window.innerWidth - ut) / window.innerWidth;
  }
  function me(a, m) {
    var M;
    if (!a)
      return !1;
    let h = a;
    const _ = (M = window.getSelection()) == null ? void 0 : M.toString(), E = c.value ? ae(c.value.$el, i.value) : null, B = /* @__PURE__ */ new Date();
    if (h.hasAttribute("data-vaul-no-drag") || h.closest("[data-vaul-no-drag]"))
      return !1;
    if (i.value === "right" || i.value === "left")
      return !0;
    if (S.value && B.getTime() - S.value.getTime() < 500)
      return !1;
    if (E !== null && (i.value === "bottom" ? E > 0 : E < 0))
      return !0;
    if (_ && _.length > 0)
      return !1;
    if (j.value && B.getTime() - j.value.getTime() < w.value && E === 0 || m)
      return j.value = B, !1;
    for (; h; ) {
      if (h.scrollHeight > h.clientHeight) {
        if (h.scrollTop !== 0)
          return j.value = /* @__PURE__ */ new Date(), !1;
        if (h.getAttribute("role") === "dialog")
          return !0;
      }
      h = h.parentNode;
    }
    return !0;
  }
  function Be(a) {
    !s.value && !H.value || c.value && !c.value.$el.contains(a.target) || (p.value = !0, L.value = /* @__PURE__ */ new Date(), a.target.setPointerCapture(a.pointerId), z.value = N(i.value) ? a.clientY : a.clientX);
  }
  function He(a) {
    var m, h, _, E, B, M;
    if (c.value && p.value) {
      const J = i.value === "bottom" || i.value === "right" ? 1 : -1, F = (z.value - (N(i.value) ? a.clientY : a.clientX)) * J, ee = F > 0, we = H.value && !s.value && !ee;
      if (we && fe.value === 0)
        return;
      const ue = Math.abs(F), he = document.querySelector("[vaul-drawer-wrapper]");
      let k = ue / ve.value;
      const be = _e(ue, ee);
      if (be !== null && (k = be), we && k >= 1 || !W.value && !me(a.target, ee))
        return;
      if ((m = c == null ? void 0 : c.value) == null || m.$el.classList.add(Se), W.value = !0, $((h = c.value) == null ? void 0 : h.$el, {
        transition: "none"
      }), $((_ = D.value) == null ? void 0 : _.$el, {
        transition: "none"
      }), H.value && Ae({ draggedDistance: F }), ee && !H.value) {
        const I = at(F), te = Math.min(I * -1, 0) * J;
        $((E = c.value) == null ? void 0 : E.$el, {
          transform: N(i.value) ? `translate3d(0, ${te}px, 0)` : `translate3d(${te}px, 0, 0)`
        });
        return;
      }
      const je = 1 - k;
      if ((pe.value || y.value && fe.value === y.value - 1) && (n(k), $(
        (B = D.value) == null ? void 0 : B.$el,
        {
          opacity: `${je}`,
          transition: "none"
        },
        !0
      )), he && D.value && l.value) {
        const I = Math.min(G() + k * (1 - G()), 1), te = 8 - k * 8, $e = Math.max(0, 14 - k * 14);
        $(
          he,
          {
            borderRadius: `${te}px`,
            transform: N(i.value) ? `scale(${I}) translate3d(0, ${$e}px, 0)` : `scale(${I}) translate3d(${$e}px, 0, 0)`,
            transition: "none"
          },
          !0
        );
      }
      if (!H.value) {
        const I = ue * J;
        $((M = c.value) == null ? void 0 : M.$el, {
          transform: N(i.value) ? `translate3d(0, ${I}px, 0)` : `translate3d(${I}px, 0, 0)`
        });
      }
    }
  }
  function ge() {
    var h;
    if (!c.value)
      return;
    const a = document.querySelector("[vaul-drawer-wrapper]"), m = ae(c.value.$el, i.value);
    $(c.value.$el, {
      transform: "translate3d(0, 0, 0)",
      transition: `transform ${g.DURATION}s cubic-bezier(${g.EASE.join(",")})`
    }), $((h = D.value) == null ? void 0 : h.$el, {
      transition: `opacity ${g.DURATION}s cubic-bezier(${g.EASE.join(",")})`,
      opacity: "1"
    }), l.value && m && m > 0 && r.value && $(
      a,
      {
        borderRadius: `${De}px`,
        overflow: "hidden",
        ...N(i.value) ? {
          transform: `scale(${G()}) translate3d(0, 14px, 0)`,
          transformOrigin: "top"
        } : {
          transform: `scale(${G()}) translate3d(14px, 0, 0)`,
          transformOrigin: "left"
        },
        transitionProperty: "transform, border-radius",
        transitionDuration: `${g.DURATION}s`,
        transitionTimingFunction: `cubic-bezier(${g.EASE.join(",")})`
      },
      !0
    );
  }
  function K() {
    var a;
    c.value && (u(), $(c.value.$el, {
      transform: N(i.value) ? `translate3d(0, ${i.value === "bottom" ? "100%" : "-100%"}, 0)` : `translate3d(${i.value === "right" ? "100%" : "-100%"}, 0, 0)`,
      transition: `transform ${g.DURATION}s cubic-bezier(${g.EASE.join(",")})`
    }), $((a = D.value) == null ? void 0 : a.$el, {
      opacity: "0",
      transition: `opacity ${g.DURATION}s cubic-bezier(${g.EASE.join(",")})`
    }), re(!1), window.setTimeout(() => {
      f.value = !1, r.value = !1;
    }, 300), window.setTimeout(() => {
      H.value && (A.value = H.value[0]);
    }, g.DURATION * 1e3));
  }
  ke(() => {
    if (!r.value && l.value && Fe) {
      const a = setTimeout(() => {
        ne(document.body);
      }, 200);
      return () => clearTimeout(a);
    }
  }), Te(() => {
    re(!1), xe();
  });
  function Ue(a) {
    if (!p.value || !c.value)
      return;
    c.value.$el.classList.remove(Se), W.value = !1, p.value = !1, Z.value = /* @__PURE__ */ new Date();
    const m = ae(c.value.$el, i.value);
    if (!me(a.target, !1) || !m || Number.isNaN(m) || L.value === null)
      return;
    const h = Z.value.getTime() - L.value.getTime(), _ = z.value - (N(i.value) ? a.clientY : a.clientX), E = Math.abs(_) / h;
    if (E > 0.05 && (T.value = !0, window.setTimeout(() => {
      T.value = !1;
    }, 200)), H.value) {
      const M = i.value === "bottom" || i.value === "right" ? 1 : -1;
      Ee({
        draggedDistance: _ * M,
        closeDrawer: K,
        velocity: E,
        dismissible: s.value
      }), d(!0);
      return;
    }
    if (i.value === "bottom" || i.value === "right" ? _ > 0 : _ < 0) {
      ge(), d(!0);
      return;
    }
    if (E > Re) {
      K(), d(!1);
      return;
    }
    const B = Math.min(
      c.value.$el.getBoundingClientRect().height ?? 0,
      window.innerHeight
    );
    if (m >= B * R.value) {
      K(), d(!1);
      return;
    }
    d(!0), ge();
  }
  Y(r, (a) => {
    a && (S.value = /* @__PURE__ */ new Date(), re(!0)), e(a);
  }, { immediate: !0 }), Y(b, (a) => {
    a ? (r.value = a, o.value = !0) : K();
  }, { immediate: !0 });
  function re(a) {
    const m = document.querySelector("[vaul-drawer-wrapper]");
    !m || !l.value || (a ? ($(document.body, {
      background: document.body.style.backgroundColor || document.body.style.background
    }), $(
      document.body,
      {
        background: "black"
      },
      !0
    ), $(m, {
      borderRadius: `${De}px`,
      overflow: "hidden",
      ...N(i.value) ? {
        transform: `scale(${G()}) translate3d(0, 14px, 0)`,
        transformOrigin: "top"
      } : {
        transform: `scale(${G()}) translate3d(14px, 0, 0)`,
        transformOrigin: "left"
      },
      transitionProperty: "transform, border-radius",
      transitionDuration: `${g.DURATION}s`,
      transitionTimingFunction: `cubic-bezier(${g.EASE.join(",")})`
    })) : (ne(m, "overflow"), ne(m, "transform"), ne(m, "borderRadius"), $(m, {
      transitionProperty: "transform, border-radius",
      transitionDuration: `${g.DURATION}s`,
      transitionTimingFunction: `cubic-bezier(${g.EASE.join(",")})`
    })));
  }
  function Me(a) {
    var _, E;
    const m = a ? (window.innerWidth - q) / window.innerWidth : 1, h = a ? -q : 0;
    X.value && window.clearTimeout(X.value), $((_ = c.value) == null ? void 0 : _.$el, {
      transition: `transform ${g.DURATION}s cubic-bezier(${g.EASE.join(",")})`,
      transform: `scale(${m}) translate3d(0, ${h}px, 0)`
    }), !a && ((E = c.value) != null && E.$el) && (X.value = window.setTimeout(() => {
      var M, J;
      const B = ae((M = c.value) == null ? void 0 : M.$el, i.value);
      $((J = c.value) == null ? void 0 : J.$el, {
        transition: "none",
        transform: N(i.value) ? `translate3d(0, ${B}px, 0)` : `translate3d(${B}px, 0, 0)`
      });
    }, 500));
  }
  function Ve(a) {
    var B;
    if (a < 0)
      return;
    const m = N(i.value) ? window.innerHeight : window.innerWidth, h = (m - q) / m, _ = h + a * (1 - h), E = -q + a * q;
    $((B = c.value) == null ? void 0 : B.$el, {
      transform: N(i.value) ? `scale(${_}) translate3d(0, ${E}px, 0)` : `scale(${_}) translate3d(${E}px, 0, 0)`,
      transition: "none"
    });
  }
  function Le(a) {
    var E;
    const m = N(i.value) ? window.innerHeight : window.innerWidth, h = a ? (m - q) / m : 1, _ = a ? -q : 0;
    a && $((E = c.value) == null ? void 0 : E.$el, {
      transition: `transform ${g.DURATION}s cubic-bezier(${g.EASE.join(",")})`,
      transform: N(i.value) ? `scale(${h}) translate3d(0, ${_}px, 0)` : `scale(${h}) translate3d(${_}px, 0, 0)`
    });
  }
  return {
    open: b,
    isOpen: r,
    modal: v,
    keyboardIsOpen: U,
    hasBeenOpened: o,
    isVisible: f,
    drawerRef: c,
    drawerHeightRef: ve,
    overlayRef: D,
    isDragging: p,
    dragStartTime: L,
    isAllowedToDrag: W,
    snapPoints: H,
    activeSnapPoint: A,
    pointerStart: z,
    dismissible: s,
    snapPointsOffset: Ne,
    direction: i,
    shouldFade: pe,
    fadeFromIndex: y,
    shouldScaleBackground: l,
    onPress: Be,
    onDrag: He,
    onRelease: Ue,
    closeDrawer: K,
    onNestedDrag: Ve,
    onNestedRelease: Le,
    onNestedOpenChange: Me,
    emitClose: u,
    emitDrag: n,
    emitRelease: d,
    emitOpenChange: e,
    nested: O
  };
}
const dt = /* @__PURE__ */ le({
  __name: "DrawerRoot",
  props: {
    activeSnapPoint: { default: void 0 },
    closeThreshold: { default: it },
    shouldScaleBackground: { type: Boolean, default: void 0 },
    scrollLockTimeout: { default: rt },
    fixed: { type: Boolean, default: void 0 },
    dismissible: { type: Boolean, default: !0 },
    modal: { type: Boolean, default: !0 },
    open: { type: Boolean, default: void 0 },
    defaultOpen: { type: Boolean, default: void 0 },
    nested: { type: Boolean, default: !1 },
    direction: { default: "bottom" },
    snapPoints: { default: void 0 },
    fadeFromIndex: { default: void 0 }
  },
  emits: ["drag", "release", "close", "update:open", "update:activeSnapPoint"],
  setup(t, { expose: n, emit: d }) {
    const u = t, e = d;
    Ie();
    const b = V(() => u.fadeFromIndex ?? (u.snapPoints && u.snapPoints.length - 1)), s = ye(u, "open", e, {
      defaultValue: u.defaultOpen,
      passive: u.open === void 0
    }), O = ye(u, "activeSnapPoint", e, {
      passive: u.activeSnapPoint === void 0
    }), x = {
      emitDrag: (y) => e("drag", y),
      emitRelease: (y) => e("release", y),
      emitClose: () => e("close"),
      emitOpenChange: (y) => {
        e("update:open", y);
      }
    }, { closeDrawer: v, hasBeenOpened: l, modal: w, isOpen: R } = nt(
      ct({
        ...x,
        ...qe(u),
        activeSnapPoint: O,
        fadeFromIndex: b,
        open: s
      })
    );
    function A(y) {
      if (s.value !== void 0) {
        x.emitOpenChange(y);
        return;
      }
      y ? (l.value = !0, R.value = y) : v();
    }
    return n({
      open: R
    }), (y, i) => (oe(), ie(C(Je), {
      open: C(R),
      modal: C(w),
      "onUpdate:open": A
    }, {
      default: se(() => [
        ce(y.$slots, "default", { open: C(R) })
      ]),
      _: 3
    }, 8, ["open", "modal"]));
  }
}), pt = /* @__PURE__ */ le({
  __name: "DrawerRootNested",
  props: {
    activeSnapPoint: {},
    closeThreshold: {},
    shouldScaleBackground: { type: Boolean },
    scrollLockTimeout: {},
    fixed: { type: Boolean },
    dismissible: { type: Boolean },
    modal: { type: Boolean },
    open: { type: Boolean },
    defaultOpen: { type: Boolean },
    nested: { type: Boolean },
    direction: {},
    snapPoints: {},
    fadeFromIndex: {}
  },
  emits: ["drag", "release", "close", "update:open", "update:activeSnapPoint"],
  setup(t, { emit: n }) {
    const d = t, u = n, { onNestedDrag: e, onNestedOpenChange: b, onNestedRelease: s } = de();
    function O() {
      b(!1);
    }
    function x(w) {
      e(w);
    }
    function v(w) {
      w && b(w), u("update:open", w);
    }
    const l = Ke(d, u);
    return (w, R) => (oe(), ie(dt, Ye(C(l), {
      nested: "",
      onClose: O,
      onDrag: x,
      onRelease: C(s),
      "onUpdate:open": v
    }), {
      default: se(() => [
        ce(w.$slots, "default")
      ]),
      _: 3
    }, 16, ["onRelease"]));
  }
}), mt = /* @__PURE__ */ le({
  __name: "DrawerOverlay",
  setup(t) {
    const { overlayRef: n, snapPoints: d, isVisible: u, isOpen: e, shouldFade: b } = de(), s = V(() => {
      var O;
      return d && (((O = d.value) == null ? void 0 : O.length) ?? 0) > 0;
    });
    return (O, x) => (oe(), ie(C(Qe), {
      ref_key: "overlayRef",
      ref: n,
      "vaul-drawer-visible": C(u) ? "true" : "false",
      "vaul-overlay": "",
      "vaul-snap-points": C(e) && s.value ? "true" : "false",
      "vaul-snap-points-overlay": C(e) && C(b) ? "true" : "false"
    }, null, 8, ["vaul-drawer-visible", "vaul-snap-points", "vaul-snap-points-overlay"]));
  }
}), gt = /* @__PURE__ */ le({
  __name: "DrawerContent",
  setup(t) {
    const {
      open: n,
      isOpen: d,
      isVisible: u,
      snapPointsOffset: e,
      drawerRef: b,
      onPress: s,
      onDrag: O,
      onRelease: x,
      modal: v,
      emitOpenChange: l,
      dismissible: w,
      keyboardIsOpen: R,
      closeDrawer: A,
      direction: y
    } = de(), i = V(() => e.value && e.value.length > 0 ? `${e.value[0]}px` : "0");
    function r(o) {
      if (!v.value || o.defaultPrevented) {
        o.preventDefault();
        return;
      }
      R.value && (R.value = !1), o.preventDefault(), w.value && l(!1), !(!w.value || n.value !== void 0) && A();
    }
    return Y(
      d,
      (o) => {
        o && setTimeout(() => {
          u.value = !0;
        }, 1);
      },
      { immediate: !0 }
    ), (o, f) => (oe(), ie(C(Ze), {
      ref_key: "drawerRef",
      ref: b,
      "vaul-drawer": "",
      "vaul-drawer-direction": C(y),
      "vaul-drawer-visible": C(u) ? "true" : "false",
      style: Xe({ "--snap-point-height": i.value }),
      onPointerdown: C(s),
      onPointermove: C(O),
      onPointerup: C(x),
      onPointerDownOutside: r,
      onEscapeKeyDown: f[0] || (f[0] = (p) => {
        C(w) || p.preventDefault();
      })
    }, {
      default: se(() => [
        ce(o.$slots, "default")
      ]),
      _: 3
    }, 8, ["vaul-drawer-direction", "vaul-drawer-visible", "style", "onPointerdown", "onPointermove", "onPointerup"]));
  }
});
export {
  bt as DrawerClose,
  gt as DrawerContent,
  $t as DrawerDescription,
  mt as DrawerOverlay,
  yt as DrawerPortal,
  dt as DrawerRoot,
  pt as DrawerRootNested,
  Dt as DrawerTitle,
  St as DrawerTrigger
};
